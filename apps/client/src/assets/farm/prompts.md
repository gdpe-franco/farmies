...
The cleaned result now lives in `cow.aseprite`: sixteen 256px frames with separate `body` and `head-mask` layers, five animation tags, and one face slice per frame. The local Aseprite exporter produces the transparent PNG atlases and JSON consumed by PixiJS. Both photos and pixel-feature placeholders use the same authored mask.
...
The generated RGB output contained a neutral checkerboard. That one-time cleanup was baked into the editable Aseprite source; runtime assets now have genuine alpha and exported face masks, so private photos cover the exact authored head pixels while leaving ears and outlines intact.
